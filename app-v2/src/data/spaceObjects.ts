/**
 * Satellites, comets, asteroids and alien craft.
 * Ported verbatim from the v1 `js/spaceObjects.js`; only types were added.
 */
import type {
  SatelliteData,
  CometData,
...
// Famous Asteroids
export const ASTEROIDS: Record<string, AsteroidData> = {
    ceres: {
        name: "Ceres",
        type: "Dwarf Planet",
        diameter: 939,
...
export const ALIEN_SHIPS: AlienShipData[] = [
    {
        id: "scout",
        name: "Scout Vessel",
        type: "scout",
...
export const SPACE_DEBRIS: { density: number; minOrbit: number; maxOrbit: number; color: number; size: number } = {
...
export const METEOR_CONFIG: { frequency: number; speed: number; trailLength: number; colors: number[]; size: number } = {
...
export const STATION_PARTS: {
  solarPanel: { width: number; height: number; depth: number; color: number }
  module: { radius: number; length: number; color: number }
  antenna: { size: number; color: number }
} = {
    solarPanel: {
        width: 0.4,
        height: 0.02,
...
export const SPACE_FACTS: Record<string, string[]> = {
    satellites: [
